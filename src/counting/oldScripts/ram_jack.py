...
def weight_variable(shape):
    ...
    # wts = tf.get_variable(name='weights', shape=shape,
    #                       initializer=tf.random_uniform_initializer(minval=-.1, maxval=.1, seed=None, dtype=tf.float32))
    # return wts
...
def calc_reward(outputs):
    ...
    # h_b_out = weight_variable((cell_out_size, 1))
    # b = tf.matmul(outputs, h_b_out)
    # b_tiled = b
    ...
    baselineErr = tf.reduce_mean(R - b_tiled)
    ...
    return cost, reward, max_p_y, correct_y, train_op, baselineErr, b
...
with tf.Graph().as_default():
    ...
    #
    cost, reward, predicted_labels, correct_labels, train_op, baselineErr, b = calc_reward(outputs)

    tf.scalar_summary("2.reward", reward)
    tf.scalar_summary("1.cost", cost)
    tf.scalar_summary("4.mean(R-b)", baselineErr)
    tf.scalar_summary("3.b", b)
