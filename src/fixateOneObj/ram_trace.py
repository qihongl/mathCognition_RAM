...
def calc_reward(outputs):
    ...
    return cost, reward, max_p_y, correct_y, sampled_locs, train_op
...
with tf.Graph().as_default():
    ...
    #
    cost, reward, predicted_labels, correct_labels, cur_sampled_locs, train_op = calc_reward(outputs)
    ...
    if eval_only:
        ...
        for step in xrange(start_step + 1, max_iters):
            ...
            correct_labels_fetched, cur_sampled_locs_fetched, f_glimpse_images_fetched = results


            # draw =  0
            # print cur_sampled_locs_fetched
            # print np.shape(cur_sampled_locs_fetched)
            # print cur_sampled_locs_fetched[0,:,:]
            # print np.shape(cur_sampled_locs_fetched[0,:,0])
            # print np.shape(cur_sampled_locs_fetched[0,:,1])
            # sys.exit('STOP')
            ...
            if step % 20 == 0:
                ...
                if draw:
                    ...
                    cur_sampled_locs_fetched = np.round(((cur_sampled_locs_fetched + 1) / 2.0) * mnist_size)
                    # visualize the trace of successive glimpses
                    plt.plot(cur_sampled_locs_fetched[0,:,0], cur_sampled_locs_fetched[0,:,1], '-o', color='lawngreen')
                    plt.plot(cur_sampled_locs_fetched[0,-1,0], cur_sampled_locs_fetched[0,-1,1], 'o',color='red')
                    fig.canvas.draw()
