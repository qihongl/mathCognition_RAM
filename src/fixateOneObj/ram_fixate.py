...
n_glimpses = 5                # number of glimpses
...
def get_next_input(output, i):
    ...
    # temp = tf.sigmoid(tf.matmul(output, b_weights1) + bias_5)
    # baseline = tf.matmul(temp, b_weights2) + bias_5
...
def calc_reward(outputs, imgBatch):
    ...
    imgs = tf.reshape(imgBatch, [batch_size, mnist_size, mnist_size])
    ...
    # preallocate for R
    R = tf.constant(np.zeros(batch_size), name='R')
    zeroVector = tf.cast(tf.constant(np.zeros(batch_size)), tf.float32)
    batchIdx = tf.constant(np.arange(batch_size), dtype=tf.int32)
    batchIdx = tf.reshape(batchIdx, [batch_size, 1])

    for i in xrange(n_glimpses):  # loop over glimpses
        # get the location coordinates for all examples in the batch
        ithGlmpCoords = tf.slice(glmpCoords, [0, i, 0], [batch_size, 1, 2])
    ...
    R_orig = R;
    reward = tf.reduce_mean(R)  # mean reward over batch and n_glimpses
    ''''''
    ...
    # J = tf.log(p_loc + 1e-5) * (R - no_grad_b)
    # print R
    # print p_loc
    # sys.exit('STOP')
    ...
    return cost, reward, train_op, p_loc, R_orig, tf.reduce_mean(b),tf.reduce_mean(R-b)
...
def toMnistCoordinates(coordinate_tanh):
    ...
    '''
    return ((coordinate_tanh + 1) / 2.0) * mnist_size
...
with tf.Graph().as_default():
    ...
    #
    cost, reward, train_op, p_loc, R, avg_b, rminusb = calc_reward(outputs, inputs_placeholder)
    ...
    if eval_only:
        ...
        for step in xrange(start_step + 1, max_iters):
            ...
            feed_dict = {inputs_placeholder: nextX, labels_placeholder: nextY, \
                         onehot_labels_placeholder: dense_to_one_hot(nextY)}
            fetches = [train_op, cost, reward, glimpse_images, p_loc, mean_locs, sampled_locs, R]
            # feed them to the model
            results = sess.run(fetches, feed_dict=feed_dict)

            _, cost_fetched, reward_fetched, f_glimpse_images_fetched, \
            p_loc_fetched, mean_locs_fetched, sampled_locs_fetched, R_fetched  = results
            ...
            if step % 20 == 0:
                ...
                if draw:
                    ...
                    # visualize the trace of successive n_glimpses (note that x and y coordinates are "flipped")
                    ...
                    # print np.round(R_fetched[0])
                    # sys.exit('STOP')
                    for y in xrange(n_glimpses):
                        txt.set_text('Fixation, R = %f \nSTEP: %i/%i'
                                     % (R_fetched[0], (y + 1), n_glimpses))
